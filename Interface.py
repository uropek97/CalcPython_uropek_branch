...
def start():
    ...
    flag_label.place(relx=0.17, rely=0.02, anchor='n')  # Позиционирование текста
    flag = tk.Checkbutton(variable=mode, command=lambda: change_mode(mode.get(), entries))
    ...
    first_entry_rac.place(relx=0.5, rely=0.13, anchor='n')  # Позиционирование окна ввода занчения
    ...
    second_entry_rac.place(relx=0.5, rely=0.37, anchor='n')  # Позиционирование окна ввода занчения
...
def send2(operation, mode, entries, result_label):
    ...
    result_label.config(text="Ваш результат: " + str(result))  # вывод результата на экран
